...
export const actFetchTasksRequest = ()=>{
	return (dispatch) =>{
		return callApi('providers','GET',null).then(res => {
			if(res !=null){
...
export const deleteTaskRequest = (id)=>{
	return (dispatch) =>{
		return callApi(`providers/${id}`,'DELETE',null).then(res =>{
...
export const saveTasksRequest = (task) =>{
	return (dispatch) =>{
		if(!task.id){
			return callApi('providers','POST',task).then(res =>{
				dispatch(saveTask(task));
			})
		}
		else{
			return callApi(`providers/${task.id}`,'PUT',task).then(res =>{
				dispatch(saveTask(task));
			})
		}
	}
